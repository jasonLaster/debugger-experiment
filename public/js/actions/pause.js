...
function paused(pauseInfo) {
  return ({ dispatch, getState, threadClient }) => {
    ...
    const source = getSource(getState(), pause.frame.location.sourceId);
    dispatch(selectSource(source.toJS()));
...
function selectFrame(frame) {
  return ({ dispatch }) => {
    // TODO: `frame.source` is created by the Frames component and we
    // need to figure out how to normalize this data better. We should
    // change `selectSource` to just take an ID.
    dispatch(selectSource(frame.source, { line: frame.location.line }));
    dispatch({
