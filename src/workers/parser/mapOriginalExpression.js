...
// input `var a = 3` => `if (!window.hasOwnProperty("a")) window.a = 3 `
function globalizeAssignment(node) {
  return node.declarations.map(({ id, init }) => {
    if (init) {
...
export default function mapOriginalExpression(
  ...
  t.traverse(ast, (node, ancestors) => {
    ...
    if (
      replacements.size > 0 &&
      (t.isIdentifier(node) || t.isThisExpression(node))
    ) {
    ...
    if (t.isVariableDeclaration(node) && !t.isBlockStatement(parentNode)) {
      const parts = globalizeAssignment(node);
